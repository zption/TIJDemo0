package com.zping.lib_thinking_in_java.t17_containers;//: containers/CollectionMethods.java
// Things you can do with all Collections.

import com.zping.lib_thinking_in_java.net.mindview.util.Countries;
...
public class CollectionMethods {
    public static void main(String[] args) {
        ...
        String[] str = c.toArray(new String[0]);// 把c 中的元素都放到 new String[0] new出来的数组中，并返回给 str
        // Find max and min elements; this means
        // different things depending on the way
        // the Comparable interface is implemented:
        print("Collections.max(c) = " + Collections.max(c));// ten
        print("Collections.min(c) = " + Collections.min(c));// ALGERIA
        // Add a Collection to another Collection
        Collection<String> c2 = new ArrayList<String>();
        c2.addAll(Countries.names(6));
        c.addAll(c2);
        print(c);// size == 14
        c.remove(Countries.DATA[0][0]);// remove 只会移除发现的第一 后面即使有 也不会移除
        print(c);// size == 13
        c.remove(Countries.DATA[1][0]);
        print(c);// size == 12
        // Remove all components that are
        // in the argument collection:
        c.removeAll(c2);// 所有的能匹配到的都移除
        print(c);// size == 2
        c.addAll(c2);
        print(c);// size == 8
        ...
        c2.removeAll(c3);
        print("c2.isEmpty() = " + c2.isEmpty());// empty        c = new ArrayList<String>();
        c.addAll(Countries.names(6));
        print(c);
